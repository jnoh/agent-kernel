...
### Status
Resolved. Spec 0015 moved tools out of the distro entirely: the six filesystem tools now live in a `kernel-workspace-local` library crate, the daemon builds a `ToolsetPool` at startup from `[[toolset]]` manifest entries, and `ConnectionRouter` snapshots `pool.tools_for_session()` at session create. The `RegisterTools`, `ExecuteTool`, `ToolResult`, and `ToolSchema` protocol variants are deleted; `proxy_tool.rs` and `in_process.rs` are gone; dispatch is kernel-owned, and a new `KernelEvent::ToolCompleted` notifies frontends when a tool finishes. The `ToolSource` enum sketched above never materialized — the seam is instead the `ToolSet` trait plus a `FactoryRegistry` keyed on `kind`. Spec 0016 completed the MCP transport half: the sole factory registered in `default_registry()` is now `"mcp.stdio" → kernel_core::mcp_stdio::from_entry`, which spawns a subprocess (typically `kernel-workspace-local`) and proxies `tools/call` over newline-delimited JSON-RPC 2.0 on the child's stdio. The daemon no longer depends on `kernel-workspace-local` as a library crate. Streaming tool output crosses the socket via `KernelEvent::ToolOutputChunk`. Adding a third-party MCP server is now "ship the binary, add a `[[toolset]]` entry" — zero kernel changes.
