...
/// A tool that tracks all inputs it received and returns scripted outputs.
...
impl RecordingTool {
    fn new(name: &str, caps: &[&str], outputs: Vec<ToolOutput>) -> Self {
        Self {
            name: name.into(),
...
struct RecordingFrontend {
    ...
    tool_results: Mutex<Vec<serde_json::Value>>,
...
impl FrontendInterface for RecordingFrontend {
    ...
    fn on_tool_call(&self, name: &str, _input: &serde_json::Value) {
        self.tool_calls.lock().unwrap().push(name.into());
    }
    fn on_tool_result(&self, _name: &str, result: &ToolOutput) {
...
fn session_config(policy: Policy) -> SessionConfig {
    SessionConfig {
